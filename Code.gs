...
function saveSignup(p) {
  ...
  const newId = lastRow; // ID = Zeilennummer - 1 (Header)
  sh.appendRow([newId, p.tag, p.name, p.schicht, p.aufgabe, new Date()]);
  return { ok: true };
}
...
function removeSignup(p) {
  const sh = SS.getSheetByName(SH_SIGNUP);
  const rows = sh.getDataRange().getValues();
  // Schema: ID|Tag|Name|Schicht|Aufgabe|Timestamp
  for (let i = rows.length - 1; i >= 1; i--) {
    if (String(rows[i][1]) === String(p.tag) && rows[i][2] === p.name) {
      sh.deleteRow(i + 1);
      return { ok: true };
    }
  }
